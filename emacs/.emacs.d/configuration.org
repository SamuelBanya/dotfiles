...
#+end_src
* Work Specific
** Add Hydra to clone standard 'goof' Git repos within '~/hub/worknotes_snyk/testing' directory
#+begin_src emacs-lisp
...
** UI
*** Add '~/.emacs.d/themes/replace-colorthemes/' to load path to add additional themes from 'replace-colorthemes' GitHub repo
#+begin_src emacs-lisp
...
#+end_src
*** Force zoom-in factor to be the same for all buffers when zooming in and out
#+begin_src emacs-lisp
  ;; Taken from here:
...
#+end_src
** Terminal
*** Add 'libvterm' to replace 'ansi-term' for better compatibility for ncurses terminal programs
#+BEGIN_SRC emacs-lisp
  (use-package vterm
    :ensure t
    :config
...
#+END_SRC
*** Force 'vterm-max-scrollback' variable to have 100,000 lines of scrollback for easier log viewing in 'vterm'
#+begin_src emacs-lisp
  (setq vterm-max-scrollback 100000)
#+end_src
** Git
*** 'magit' to make Git 10x easier to use
#+BEGIN_SRC emacs-lisp
  (use-package magit
    :ensure t
    :config
...
#+end_src
*** Add 'json-mode' for support for .json files
#+begin_src emacs-lisp
...
#+BEGIN_SRC emacs-lisp
  ...
  (with-eval-after-load 'dired
	...
	(find-alternate-file ".."))))
#+END_SRC
*** Use 'ranger.el' as a replacement for 'Dired'
#+begin_src emacs-lisp
  (use-package ranger
    :ensure t)
#+end_src
...
** Exporting HTML From Org Docs
*** Add ability to create buttons
#+begin_src emacs-lisp
